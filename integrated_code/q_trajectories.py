...
def constant_function(x, const_value):
    return const_value
...
class System:  
    def __init__(self, drivingH, initialState, timeList, *, lindbladList = L_dummy, FList = F_dummy, uMatrix = [], mMatrix = [], oMatrix = [], HMatrix = [], amp= None):
        ...
        #########################################################
        ## Both Lindblad and feedback operator must be two argument functions, beign its arguments time(t) and state(rho)
        if not (type(lindbladList).__name__  in ['list', 'partial', 'function']) and (lindbladList != L_dummy):
            raise ValueError('Lindblad channels must be passed as a function or iterable')

        if not (type(FList).__name__  in ['list', 'partial', 'function']) and (FList != F_dummy):
            raise ValueError('Feedback operators must be passed as a function or iterable')

        ## If they are not functions with these characteristics we redefine them as such
        if (type(lindbladList).__name__ == 'list') & (lindbladList != L_dummy):
            lindbladList = partial(constant_function, const_value=lindbladList)

        if (type(FList).__name__ == 'list') & (FList != F_dummy):
            FList = partial(constant_function, const_value=FList)
    ...
    def diffusiveRhoTrajectory(self, seed_nn = 1, method=1, last_point=False):
        ## Initial state
        rho = self.initialRho
        
        ## 0 - Heun, 1 - Euler, 3 - Milstein
        dict_methods = {0:self.diffusiveRhoHeunStep, 1:self.diffusiveRhoEulerStep, 2:self.diffusiveRhoMilsteinStep}
        step_method = dict_methods[method]
        ...
        if self.nonfixedUnraveling == 0:
            ...
            if not last_point:
                ...
                for it in range(1, self.maxiter):
                    rho = rho + step_method(it, rho, dz[it])
    ...
    def diffusiveRhoAverage(self, n_trajectories, traj_type = 'density_matrix', method = 'euler', parallelfor = True, last_point = False):
        rng = default_rng()
        ########################################################
        dict_methods = {'heun':0, 'euler':1, 'milstein':2}
        try:
            Me = dict_methods[method]
        except:
            Me = 1
            warnings.warn("Method not supported. Euler selected")
        ########################################################
        if traj_type == 'vector':
            if parallelfor == True:
                rho_ = np.zeros((self.maxiter, self.dimH, self.dimH), dtype = np.complex128) 
                split_lenght = np.ceil(n_trajectories/(2*cpu_count()))
                trajectory_seeds = rng.choice(10*n_trajectories, size=n_trajectories, replace=False)
                spliting = np.array_split(trajectory_seeds, split_lenght)
                for seg in spliting:
            ...
            if parallelfor == True:
                ...
                if not last_point:
                    rho_ = np.zeros((self.maxiter, self.dimH, self.dimH), dtype = np.complex128) 
                    for seg in spliting:
                        seg_rho_traj = self.segmented_parallel_run(partial(self.diffusiveRhoTrajectory, method = Me, last_point = False), seg) 
                        ##################################################################
                        for it in range(self.maxiter):
                            rho_i = np.zeros((self.dimH,self.dimH), dtype = np.complex128)
                            for n_seg,_ in enumerate(seg):
                                rho_i += (1./n_trajectories)*seg_rho_traj[n_seg][it]     
                            rho_[it] += rho_i
                        ##################################################################
                        del seg_rho_traj
                    return rho_
                else:
                    rho_last = np.zeros((self.dimH, self.dimH), dtype = np.complex128) 
                    for seg in spliting:
                        seg_rho_traj = self.segmented_parallel_run(partial(self.diffusiveRhoTrajectory, method = Me, last_point = True), seg) 
                        ##################################################################
                        for n_seg,_ in enumerate(seg):
                            rho_last += (1./n_trajectories)*seg_rho_traj[n_seg]    
                        ##################################################################
                        del seg_rho_traj
                    return rho_last
            else:
                rho_trajectories = []
                for i in range(n_trajectories):
                    rho_trajectories.append(self.diffusiveRhoTrajectory(seed_nn = i, method = Me, last_point = last_point))
                if not last_point:
                    rho_ = []
                    for it in range(self.maxiter):
                        rho_i = np.zeros((self.dimH,self.dimH), dtype = np.complex128)
                        for n in range(n_trajectories):
                            rho_i += (1./n_trajectories)*rho_trajectories[n][it]
                        rho_.append(rho_i)
                    return rho_
                else:
                    rho_last = np.zeros((self.dimH,self.dimH), dtype = np.complex128)
                    for n in range(n_trajectories):
                        rho_last += (1./n_trajectories)*rho_trajectories[n]
                    return rho_last   
    ...
    ######################################################################################
    def feedRhoTrajectory(self, seed_nn, method=1, last_point=False):
        ## Initial state
        rho = self.initialRho
        
        ## 0 - Heun, 1 - Euler, 2 - Milstein
        dict_methods = {0:self.feedRhoHeunStep, 1:self.feedRhoEulerStep, 2:self.feedRhoMilsteinStep}
        step_method = dict_methods[method]
    ...
    ######################################################################################
    def feedRhoAverage(self, n_trajectories, method = 'euler', parallelfor = True,  last_point = False):
        rng = default_rng()
        ########################################################
        dict_methods = {'heun':0, 'euler':1, 'milstein':2}
        try:
        ...
        if parallelfor == True:
            ...
            if not last_point:
                rho_ = np.zeros((self.maxiter, self.dimH, self.dimH), dtype = np.complex128) 
                for seg in spliting:
                    seg_rho_traj = self.segmented_parallel_run(partial(self.feedRhoTrajectory, method = Me, last_point = False), seg) 
                    ##################################################################
                    for it in range(self.maxiter):
                        rho_i = np.zeros((self.dimH,self.dimH), dtype = np.complex128)
                        for n_seg,_ in enumerate(seg):
                ...
                rho_last = np.zeros((self.dimH, self.dimH), dtype = np.complex128) 
                for seg in spliting:
                    seg_rho_traj = self.segmented_parallel_run(partial(self.feedRhoTrajectory, method = Me, last_point = True), seg) 
                    ##################################################################
                    for n_seg,_ in enumerate(seg):
                        rho_last += (1./n_trajectories)*seg_rho_traj[n_seg]    
                    ##################################################################
            ...
            rho_trajectories = []
            for i in range(n_trajectories):
                rho_trajectories.append(self.feedRhoTrajectory(seed_nn = i, method = Me, last_point = last_point))
            if not last_point:
                rho_ = []
                for it in range(self.maxiter):
                    rho_i = np.zeros((self.dimH,self.dimH), dtype = np.complex128)
    ...
    ###################################################################################
    def feedAnaliticalUncond(self, method = 'BDF', rrtol = 1e-5, aatol = 1e-5, last_point = False):
        dict_methods = {'BDF':'BDF', 'DOP853':DOP853}
        try:
            method_ = dict_methods[method]
        except:
            method_ = method
        ...
        sol = 0
        if not last_point:
            sol = solve_ivp(odefun, [self.timeList[0], self.timeList[-1]], x0, t_eval=self.timeList, method = method_, rtol = rrtol, atol = aatol)
            gc.collect()
            rho_T = [sol.y[:,i].reshape([self.dimH,self.dimH]) for i in range(len(sol.t))] 
            return rho_T
        
        else:
            sol = solve_ivp(odefun, [self.timeList[0], self.timeList[-1]], x0, t_eval=[self.timeList[-1]], method = method_, rtol = rrtol, atol = aatol)
            gc.collect()
            rho_T = [sol.y[:,i].reshape([self.dimH,self.dimH]) for i in range(len(sol.t))] 
            return rho_T[0]

    def feedAnaliticalUncond_F0(self, method = 'BDF', rrtol = 1e-5, aatol = 1e-5, eff_ = 'no_unit', last_point = False):
        dict_methods = {'BDF':'BDF', 'DOP853':DOP853}
        try:
            method_ = dict_methods[method]
        except:
    ...
    def customAnalitical(self, L, method = 'BDF', rrtol = 1e-5, aatol = 1e-5):
        dict_methods = {'BDF':'BDF', 'DOP853':DOP853}
        try:
            method_ = dict_methods[method]
        except:
    ...
    def schrodingerAnalitical(self, method = 'BDF', rrtol = 1e-5, aatol = 1e-5):
        dict_methods = {'BDF':'BDF', 'DOP853':DOP853}
        try:
            method_ = dict_methods[method]
        except:
        ...
        if self.initial_counter == 1:
            raise ValueError('Initial mixed state defined')
        psi0 = self.jumpPsi0
        sol = solve_ivp(self.standardSchrodinger_op, [self.timeList[0], self.timeList[-1]], psi0, t_eval=self.timeList, method = method_, rtol = rrtol, atol = aatol)
        gc.collect()
    ...
    #############################################################
    def lindbladAnalitical(self, method = 'BDF', rrtol = 1e-5, aatol = 1e-5, last_point = False, gamma = False, inefficient = False):
        dict_methods = {'BDF':'BDF', 'DOP853':DOP853}
        try:
            method_ = dict_methods[method]
        except:
    ...
    def VonNeumannAnaliticalRunge(self, rungesteps = 10, last_point = 0):
        ...
        if last_point == 1:
            ...
            while t < self.tmax:
                ...
                if np.linalg.norm(rho_T_aux) >= 1 + 1e-5:
                    steps += 0.5*((10)**nn)
                    print(f'Precision Incresed: t: {t}, dt -> {Dt/steps}')
                    nn += 1
                    continue
                rho_T = rho_T_aux
                t += Dt/steps
...
#             raise e
    ...
    def parallel_run(self, fun, n_trajectories, tqdm_bar = True):
        try:
            p = Pool(processes = cpu_count())
            if not tqdm_bar:
                m = p.map(fun, np.rint(100000*np.random.rand(n_trajectories)).astype(int))
                ...
                with tqdm(total=n_trajectories) as pbar:
                    m = []
                    for _ in p.map(fun, np.rint(100000*np.random.rand(n_trajectories)).astype(int)):
                        m.append(_)
                        pbar.update()
                    return m
        except KeyboardInterrupt as e:
    ...
    def segmented_parallel_run(self, fun, array, tqdm_bar = True):
        try:
            p = Pool(processes = cpu_count())
            if not tqdm_bar:
                m = p.map(fun, array)
...
### Misc functions
def parallel_run(fun, arg_list, tqdm_bar = True):
    try:
        p = Pool(processes = cpu_count())
        if not tqdm_bar:
